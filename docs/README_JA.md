...
# 分子記述子に基づく Huggins パラメータ（χ）の QSAR 予測モデル
...
2. 複数のソースからのデータセットを統合します（旧データ 323 件 + 新データ 1586 件、文献間の chi 矛盾をクリーニング・解決して **1815 件** を取得）。
3. **RDKit** を利用して、全 **約 210 個** の 2D 分子記述子 + 指紋類似度 + 差値特徴量と温度逆数項を自動計算し、**332 次元の特徴量行列** を生成します。
4. **遺伝的アルゴリズム（GA）** を使用して、332 次元の中から最適な特徴量サブセットを選択します。
...
├── 特征工程.py                 # Step 3: 全量 RDKit 記述子抽出 (332 次元)
├── 遗传.py                    # Step 4a: GA 粗選別（性能版、RFECV 自動接続用）
├── 遗传_ElasticNet.py         # Step 4a: GA 粗選別（物理版、ga_elasticnet_* を独立出力）
├── 特征筛选.py                 # Step 4b: RFECV 精選別（デフォルトで results/ga_selected_features.txt を読み込み）
...
│   ├── 43579_2022_237_MOESM1_ESM.csv  # 新規外部データセット (1586 件)
│   ├── merged_dataset.csv             # 統合後データセット (1815 件、矛盾解決済み)
│   ├── molecular_features.xlsx        # 332 次元特徴量行列
...
│   ├── ga_elasticnet_best_model.pkl # 物理版 GA 最良モデル
│   ├── ga_selected_features.txt     # GA 選別特徴量リスト
│   ├── ga_elasticnet_selected_features.txt # 物理版 GA 選別特徴量リスト
│   ├── ga_evolution_log.csv         # GA 進化ログ
│   ├── ga_elasticnet_evolution_log.csv # 物理版 GA 進化ログ
...
| Step 3：特徴量エンジニアリング | `特征工程.py` | `data/molecular_features.xlsx`（332 次元） |
| Step 4：特徴量選別 | `遗传.py`（性能版 GA）、`遗传_ElasticNet.py`（物理版 GA）、`特征筛选.py` | `results/ga_selected_features.txt`、`results/ga_elasticnet_selected_features.txt`、`data/features_optimized.xlsx` |
...
> ℹ️ 推奨の自動接続パイプラインは `遗传.py -> 特征筛选.py` です。現在、`特征筛选.py` はデフォルトで `results/ga_selected_features.txt` を読み込みます。
> `遗传_ElasticNet.py` は物理的解釈性比較のために独立した `results/ga_elasticnet_*` 結果を生成します。RFECV に接続するには、手動での特徴量リスト同期が必要です。

---

## モデリング段階（Step 5）

以下のコマンドはすべて同一のアクティブ化された Python 環境で実行されることを前提としています。`python` は現在の環境のインタープリターを指します。

### Step 5a: DNN Hyperband 自動チューニング

**スクリプト**: [`DNN_AutoTune.py`](../DNN_AutoTune.py)

Keras Tuner の Hyperband アルゴリズムを使用して、DNN の最適アーキテクチャ（1-3 層、12-64 ノード、学習率、正則化など）を探索します。

| 設定項目 | 値 |
|----------|---|
| 探索戦略 | Hyperband (Keras Tuner) |
| 探索空間 | 1-3 層, 12-64 ノード, L2 正則化, Dropout |
| データ分割 | 60% 訓練 / 20% 検証 / 20% テスト |
| 標準化 | X と y の両方に StandardScaler を使用 |
| 再学習 | 最適アーキテクチャを異なるシードで 8 回再学習、最終モデルは validation MAE で選択 |

```bash
python DNN_AutoTune.py
```

> ℹ️ `DNN_AutoTune.py` は Hyperband と同じ検証指標 `val_mae` で最終 seed を選択します。`test_r2` は使用しません。

### Step 5b: Sklearn AutoTune（推奨）

**スクリプト**: [`Sklearn_AutoTune.py`](../Sklearn_AutoTune.py)

4 つのモデル × 50 組のパラメータ × 5 分割交差検証（CV）による自動最適化：
...
4. 検証可視化（実測値 vs 予測値、残差分布、モデル比較など 4 枚の図）
...
| [`Sklearn_AutoTune.py`](../Sklearn_AutoTune.py) | 学習終了後、Sklearn 検証結果を自動出力（`final_results/sklearn/sklearn_validation_results.xlsx`、ローカル産物、Git 非追跡） |
...
**機能**: Y-Scrambling 検証。y 値を 100 回ランダムにシャッフルしてモデルを再学習し、QSAR モデルが特徴量と目的変数の関係を真に学習しているかを検証します。真のモデルの R² がランダムモデルの分布より有意に高ければ (p < 0.05)、モデルは有効です。

**出力**: `final_results/sklearn/y_randomization.png`、`final_results/sklearn/y_randomization.csv`
...
**機能**: 同一の train/test 分割を再利用した上で、DNN の `y_train/y_val` をランダムにシャッフルして再学習を繰り返し、真の DNN とランダム化 DNN のテストセット R² 分布と p 値を比較します。

**出力**: `final_results/dnn/dnn_y_randomization.csv`、`final_results/dnn/dnn_y_randomization.png`、`final_results/dnn/dnn_y_randomization_summary.txt`
...
**出力**: `final_results/dnn/dnn_validation_plots.png`、`final_results/dnn/dnn_validation_results.csv`、`final_results/dnn/dnn_feature_importance.csv`
...
| ファイル | 場所 | 説明 | 生成段階 | Git |
|----------|------|------|----------|-----|
| `43579_2022_237_MOESM1_ESM.csv` | `data/` | 外部データセット (1586 件) | 新規入力 | ✅ |
| `smiles_raw.csv` | `data/` | SMILES 照会結果 | Step 1 | ✅ |
| `smiles_cleaned.xlsx` | `data/` | 手動クリーニング後の SMILES | 手動処理 | ✅ |
| `huggins_preprocessed.xlsx` | `data/` | 前処理済みデータ (323 件) | Step 2 | ✅ |
| `merged_dataset.csv` | `data/` | 統合データセット (1815 件、矛盾解決済み) | Step 2.5 | ✅ |
| `molecular_features.xlsx` | `data/` | 332 次元特徴量行列 | Step 3 | ✅ |
| `features_optimized.xlsx` | `data/` | 選別後特徴量サブセット | Step 4 | ✅ |
| `ga_selected_features.txt` | `results/` | GA 選別特徴量リスト | Step 4 | — |
| `ga_elasticnet_selected_features.txt` | `results/` | 物理版 GA 選別特徴量リスト | Step 4 | — |
| `ga_evolution_log.csv` | `results/` | GA 進化ログ | Step 4 | — |
| `ga_elasticnet_evolution_log.csv` | `results/` | 物理版 GA 進化ログ | Step 4 | — |
...
| `sklearn_validation_plots.png` | `final_results/sklearn/` | Sklearn 検証可視化 (4 サブプロット) | Step 5 | ✅ |
...
> ℹ️ DNN 行は六組アーカイブとの横断比較用の歴史的 best run 指標です。リポジトリの最終 `best_model.keras` は validation MAE で選択されています。
> ℹ️ デフォルト表示は 9 特徴量の `pre_physics` 解釈可能ベースラインです。他の 5 組の比較実験は別途アーカイブされています。
...
# 2. 環境の作成と有効化（conda 推奨、RDKit と TensorFlow が同一インタープリターを使用することを保証）
...
# 以下の python コマンドはすべて上記の有効化された環境で実行されることを前提とします

# 4. データ統合 + 特徴量エンジニアリング + 二段階特徴量選択 + モデリング
python 合并数据集.py              # 旧データと新データの統合
python 特征工程.py                # 全量 RDKit 記述子 (332 次元)
python 遗传.py                   # 性能版 GA 粗選別（特征筛选.py と自動接続）[推奨]
# python 遗传_ElasticNet.py      # 物理版 GA 粗選別（ga_elasticnet_* を出力、RFECV 接続には手動同期が必要）[代替]
...
| 指標 | 式 | 説明 |
|------|----|------|
| **R²** | 1 - SS_res/SS_tot | 決定係数。1 に近いほど良い |
