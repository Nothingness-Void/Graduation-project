# -*- coding: utf-8 -*-
"""
遗传_ElasticNet.py - 物理解释型 GA 特征选择 (MI → GA)

流程:
  1. Mutual Information 粗筛: 缩小候选空间
  2. GA 组合搜索: 在 MI 缩减后的空间中搜索最优子集
  3. ElasticNet 评估: 保持线性物理解释性
...
输出:
    ...
    - results/ga_elasticnet_mi_ranking.csv
...
# MI 预筛选
MI_TOP_N = 100              # 默认更稳，避免 top-60 过度压缩
...
MI_PATH = os.path.join(RESULTS_DIR, f"{OUTPUT_PREFIX}_mi_ranking.csv")
...
def load_or_create_split(n_samples):
    """Load saved split if it matches current dataset metadata, otherwise recreate it."""
    expected_n = int(n_samples)
    if os.path.exists(SPLIT_INDEX_PATH):
        saved = np.load(SPLIT_INDEX_PATH)
        saved_files = set(saved.files)
        has_meta = {"n_samples", "test_size", "random_state"}.issubset(saved_files)
        has_index = {"train_idx", "test_idx"}.issubset(saved_files)

        if has_meta and has_index:
            saved_n = int(saved["n_samples"][0])
            saved_test_size = float(saved["test_size"][0])
            saved_random_state = int(saved["random_state"][0])
            train_idx = saved["train_idx"]
            test_idx = saved["test_idx"]
            index_valid = (
                len(train_idx) + len(test_idx) == expected_n
                and train_idx.min() >= 0
                and test_idx.min() >= 0
                and train_idx.max() < expected_n
                and test_idx.max() < expected_n
            )
            meta_valid = (
                saved_n == expected_n
                and np.isclose(saved_test_size, TEST_SIZE)
                and saved_random_state == RANDOM_STATE
            )
            if meta_valid and index_valid:
                print(f"  已复用 split 索引: {SPLIT_INDEX_PATH}")
                return train_idx, test_idx

        print("  检测到旧 split 与当前数据/配置不一致，已重新生成")

    all_indices = np.arange(expected_n)
...
def main():
    ...
    train_idx, test_idx = load_or_create_split(len(y))
    ...
    # 3) MI 粗筛
    print(f"\n{'='*70}")
    print("Stage 1: Mutual Information 粗筛")
    print(f"{'='*70}")
    ...
    print(f"  MI 排名已保存: {MI_PATH}")

    mi_top_names = mi_df["feature"].head(MI_TOP_N).tolist()
    for feat in MANDATORY_FEATURES:
        if feat not in mi_top_names:
            mi_top_names.append(feat)
            print(f"  必选特征 {feat} 不在 MI top-{MI_TOP_N}，已强制加入")
    ...
    if len(inv_t_rank) > 0:
        inv_t_mi = mi_df.loc[mi_df["feature"] == "Inv_T", "mi_score"].iloc[0]
        print(f"  Inv_T MI 排名: #{int(inv_t_rank[0])} (MI={inv_t_mi:.4f})")

    # 4) GA 搜索
    print(f"\n{'='*70}")
    print("Stage 2: GA 组合搜索")
    print(f"{'='*70}")
